...
def run_experiment(population, seed):
    ...
    for _ in range(N_REPEATS):
        ...
        chunked_samples = rng.choice(population, size=(N_CHUNKS, SMALL_N), replace=False)
    ...
    return np.array(boot_means), np.array(boot_stds), np.array(large_means), np.array(large_stds)

def main():
    print("Generating Problem 1 figures... (1000 repeats)")
    bernoulli_pop, exponential_pop = generate_populations()
    
    b_boot_means, b_boot_stds, b_large_means, b_large_stds = run_experiment(bernoulli_pop, SEED)
    e_boot_means, e_boot_stds, e_large_means, e_large_stds = run_experiment(exponential_pop, SEED+1)

    # FIGURE 1: Mean Estimates
    fig, axes = plt.subplots(2, 2, figsize=(10, 8))
    fig.suptitle("Mean Estimators: 20x25 vs 1x500 (Identical Distributions)", fontsize=14, fontweight="bold")
    ...
    for i, (pop, name, color, boot_m, large_m) in enumerate([
        ...
        if "Bernoulli" in name:
            ...
            ax.bar(vals, counts / POPULATION_SIZE, width=0.4, color=color, edgecolor="white")
        else:
        ...
        ax.text(0.95, 0.85, f"μ={true_mu:.3f}\nσ={true_std:.3f}", transform=ax.transAxes, ha="right", bbox=dict(boxstyle="round", facecolor="wheat", alpha=0.5))
        
        # Overlapping Means - exactly the same
        ax = axes[i, 1]
        ax.hist(boot_m, bins=40, density=True, color="#C44E52", edgecolor="white", alpha=0.6, label="Mean of 20x25 (Batch)")
        ax.hist(large_m, bins=40, density=True, color="#55A868", edgecolor="white", alpha=0.6, label="Mean of 500 (Pooled)")
    ...
    fig.suptitle("Standard Deviation Estimators over 1000 Repeats: 20x25 vs 1x500", fontsize=14, fontweight="bold")
    
    for ax, (pop, name, boot_s, large_s) in zip(axes, [
        ...
        ax.hist(boot_s, bins=40, density=True, color="#C44E52", alpha=0.6, label="Avg Var of 20 subsets of 25")
        ax.hist(large_s, bins=40, density=True, color="#55A868", alpha=0.6, label="Var of single pool of 500")
        ax.axvline(true_std, color="black", linestyle="--", lw=2, label=f"True Pop Std ({true_std:.3f})")
    ...
    fig.suptitle("Bootstrap Inference: Estimating Our Own Error from 1 Sample of N=50", fontsize=14, fontweight="bold")
    ...
    for i, (pop, name, color) in enumerate([
        ...
        # Plot 1: Mean Error
        ax = axes[i, 0]
        ax.hist(resampled_means, bins=30, density=True, color=color, alpha=0.7, edgecolor='white')
        ax.axvline(sample_mean, color='black', lw=2, label="Sample Mean")
        ax.axvline(pop.mean(), color='red', linestyle='--', lw=1.5, label="True Pop Mean")
        ax.set_title(f"{name}: Bootstrap Dist. of the Mean", fontsize=10)
        
        text_mean = f"Bootstrap SE: {np.std(resampled_means):.4f}\nTrue SE: {true_mean_se:.4f}"
        ax.text(0.95, 0.85, text_mean, transform=ax.transAxes, ha="right", bbox=dict(boxstyle="round", facecolor="wheat", alpha=0.5))
        ax.legend(fontsize=8)

        # Plot 2: Std Error
        ax = axes[i, 1]
        ax.hist(resampled_stds, bins=30, density=True, color="#9B59B6", alpha=0.7, edgecolor='white')
        ...
        text_std = f"Bootstrap SE: {np.std(resampled_stds):.4f}\nTrue SE: {true_std_se:.4f}"
        ax.text(0.95, 0.85, text_std, transform=ax.transAxes, ha="right", bbox=dict(boxstyle="round", facecolor="wheat", alpha=0.5))
        ax.legend(fontsize=8)
