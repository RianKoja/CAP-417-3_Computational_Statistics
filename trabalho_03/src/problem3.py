...
def part_a_mcs_shines():
    ...
    x_tr = rng.uniform(-4, 4, n_tr);  y_tr = x_tr**2 + rng.normal(0, noise, n_tr)
    x_te = rng.uniform(-4, 4, n_te);  y_te = x_te**2 + rng.normal(0, noise, n_te)
    ...
    ax.scatter(x_tr, y_tr, s=12, alpha=0.25, color="#4878CF", marker="o",
               label=f"Train (n={n_tr})", zorder=1)
    ax.scatter(x_te, y_te, s=12, alpha=0.35, color="#D65F5F", marker="^",
               label=f"Test (n={n_te})", zorder=1)
    ...
    for (name, m), color in zip(poly_models.items(), colors):
        ...
        ax.plot(x_plot, m.predict(x_plot), color=color,
                lw=2.5 if pv >= ALPHA else 1.2,
                ls="-" if pv >= ALPHA else "--",
                label=f"{name}  (p={pv:.2f})", zorder=3)
    ax.set_xlabel("x"); ax.set_ylabel("y")
    ax.set_title("Fitted curves\n(solid line = in MCS, dashed = excluded)")
    ax.legend(fontsize=8, loc="upper center"); ax.set_ylim(-5, 25)
...
def _gen_linear(n_pts, n_out, noise=0.1, seed=42):
    ...
    xi = rng.uniform(-5, 5, n_pts);  yi = xi + rng.normal(0, noise, n_pts)
    if n_out > 0:
        ang = rng.uniform(0, 2*np.pi, n_out);  r = rng.uniform(3, 6, n_out)
        xo = r * np.cos(ang);  yo = r * np.sin(ang)
        return np.r_[xi, xo], np.r_[yi, yo], np.r_[np.zeros(n_pts, bool), np.ones(n_out, bool)]
    return xi, yi, np.zeros(n_pts, bool)


def _gen_quadratic(n_pts, n_out, noise=0.5, seed=42):
    rng = np.random.default_rng(seed)
    xi = rng.uniform(-3, 3, n_pts);  yi = xi**2 + rng.normal(0, noise, n_pts)
    if n_out > 0:
        xo = rng.uniform(-3, 3, n_out);  yo = rng.uniform(-5, 15, n_out)
        return np.r_[xi, xo], np.r_[yi, yo], np.r_[np.zeros(n_pts, bool), np.ones(n_out, bool)]
    return xi, yi, np.zeros(n_pts, bool)
...
def _gen_exponential(n_pts, n_out, noise=0.3, seed=42):
    rng = np.random.default_rng(seed)
    xi = rng.uniform(-1, 2, n_pts);  yi = np.exp(xi) + rng.normal(0, noise, n_pts)
    if n_out > 0:
        xo = rng.uniform(-1, 2, n_out);  yo = rng.uniform(-2, 10, n_out)
        return np.r_[xi, xo], np.r_[yi, yo], np.r_[np.zeros(n_pts, bool), np.ones(n_out, bool)]
    return xi, yi, np.zeros(n_pts, bool)
...
def _fit_ls(x, y, kind):
    ...
    if kind == 'quadratic':
        m = make_poly(2); m.fit(X, y); return m
    mask = y > 0
    if mask.sum() < 2: return None
    m = LinearRegression(); m.fit(X[mask], np.log(y[mask]))
    return (m, mask)
...
def _fit_ransac(x, y, kind, seed=42):
    X = x.reshape(-1, 1)
    if kind == 'linear':
        m = RANSACRegressor(random_state=seed, min_samples=2); m.fit(X, y); return m
    if kind == 'quadratic':
    ...
    try:
        ...
        m.fit(X[mask], np.log(y[mask])); return (m, mask)
    except Exception:
        return None


def _fit_lgbm(x, y, seed=42):
    m = lgb.LGBMRegressor(random_state=seed, n_estimators=100,
                           learning_rate=0.1, max_depth=3, verbose=-1)
    m.fit(x.reshape(-1, 1), y); return m


def _fit_lgbm_ransac(x, y, seed=42):
    base = lgb.LGBMRegressor(random_state=seed, n_estimators=50,
                              learning_rate=0.1, max_depth=3,
                              min_child_samples=1, verbose=-1)
    m = RANSACRegressor(estimator=base, random_state=seed,
                        min_samples=0.5, max_trials=100)
    m.fit(x.reshape(-1, 1), y); return m
...
def _evaluate(x, y, model, is_log_space: bool):
    try:
        ...
        if is_log_space:
            ...
            y_eval = y
        sq = resid ** 2
        return {'rmse': float(np.sqrt(sq.mean())),
...
def _plot_comparison_figure(type_name, kind, datasets, models_list, pvals_list, out_path):
    """
    ...
    fig.suptitle(f"{type_name} Data — Model Fits and MCS p-values",
    ...
    for row, (label, (x, y, outlier), models, pvals) in enumerate(
        ...
        ax = axes[row, 0]
        inl = ~outlier;  outtl = outlier
        ax.scatter(x[inl],  y[inl],  s=18, alpha=0.5, color="#4878CF",
                   label="Inliers", zorder=3)
        if outtl.any():
            ax.scatter(x[outtl], y[outtl], s=25, alpha=0.6, color="#D65F5F",
                       marker="x", linewidths=1.5, label="Outliers", zorder=3)
        ax.plot(x_plot, true_fn(x_plot), "k--", lw=1.4,
                label=f"True {true_label}", zorder=2)
        ...
        for name, color in zip(MODEL_NAMES, MODEL_COLORS):
            ...
            ax.plot(x_plot, y_fit, color=color, lw=2.2,
                    ls="-" if pv >= ALPHA else "--",
                    label=f"{name} (p={pv:.2f})", zorder=4)
...
def _row_colors(values, inverse=False):
    ...
    for v in values:
        if not np.isfinite(v):
            out.append((220, 220, 220)); continue
...
def _write_typst_table(metrics_data, out_path, caption):
    ...
    for label, d in zip(row_labels, metrics_data):
        ...
        rc = _row_colors(rv); mc = _row_colors(mv); pc = _row_colors(pv, inverse=True)
        row = (f"    table.cell(align: left)[{label}],"
...
def part_b_full_comparison():
    ...
    type_specs = [
        ("Linear",      "linear",      _gen_linear,
         [(100, 0, 0), (80, 20, 1), (50, 50, 2)]),
        ("Quadratic",   "quadratic",   _gen_quadratic,
         [(100, 0, 3), (80, 20, 4), (50, 50, 5)]),
        ("Exponential", "exponential", _gen_exponential,
    ...
    for type_name, kind, gen_fn, outlier_specs in type_specs:
        ...
        for n_pts, n_out, off in outlier_specs:
            ...
            pvals_raw = {k: 0. for k in col_n}
            if all(e['losses'] is not None for e in evs):
                mn = min(len(e['losses']) for e in evs)
                ldf = pd.DataFrame({n: e['losses'][:mn]
                                     for n, e in zip(col_n, evs)})
            ...
            pvals_list.append({
                "LS":     pvals_raw.get('LS',          0.),
                "RANSAC": pvals_raw.get('RANSAC',       0.),
                "LGBM":   pvals_raw.get('LightGBM',     0.),
                "R+LGBM": pvals_raw.get('R+LightGBM',  0.),
            })
            metrics_data.append({
                'ls_rmse':         ls_ev['rmse'],
                ...
                'ls_mcs':          pvals_raw.get('LS',         0.),
                'ransac_mcs':      pvals_raw.get('RANSAC',      0.),
                'lgbm_mcs':        pvals_raw.get('LightGBM',    0.),
                'lgbm_ransac_mcs': pvals_raw.get('R+LightGBM', 0.),
            })

        # Figure
        fig_out = FIGURES_DIR / f"problem3_comparison_{kind}.svg"
        _plot_comparison_figure(type_name, kind, datasets,
                                models_list, pvals_list, fig_out)
...
def part_c_mcs_fails():
    ...
    x_in = rng.uniform(-5, 5, n_in);  y_in = x_in + rng.normal(0, 0.5, n_in)
    ang  = rng.uniform(0, 2*np.pi, n_out);  r = rng.uniform(3, 6, n_out)
    x_out = r * np.cos(ang);  y_out = r * np.sin(ang)
    x_all = np.r_[x_in, x_out];  y_all = np.r_[y_in, y_out]
    ...
    l_all   = {n: (y_all - m.predict(x_all.reshape(-1, 1)))**2 for n, m in models.items()}
    l_clean = {n: (y_in  - m.predict(x_in.reshape( -1, 1)))**2 for n, m in models.items()}
    pv_all   = run_mcs(pd.DataFrame(l_all))
    pv_clean = run_mcs(pd.DataFrame(l_clean))

    print("\nPart C — contaminated data:")
    for name in models:
        print(f"  {name:22s}  RMSE={np.sqrt(l_all[name].mean()):.3f}"
              f"  p={pv_all.get(name,0.):.3f}")
    print("Part C — clean inliers only:")
    for name in models:
        print(f"  {name:22s}  RMSE={np.sqrt(l_clean[name].mean()):.3f}"
              f"  p={pv_clean.get(name,0.):.3f}")
    ...
    ax.set_xlabel("x"); ax.set_ylabel("y")
    ax.set_title("Data and Fitted Models"); ax.legend(fontsize=7)
