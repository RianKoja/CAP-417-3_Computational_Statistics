...
The presentation itself (slides, titles, labels, figures, and tables) and all code files shall be written in English.
...
Prefer plain Python CLIs, standard Make recipes, and tools that run locally or in CI without vendor lock-in.
...
- `generate_tables.py`
  ...
  - Ljung–Box results
  - volatility summary
  - tail index estimation (e.g., via Hill estimator)
  - threshold exceedance statistics (peaks-over-threshold method)
...
- `generate_window_plots.py`
  ...
  - tail index or threshold exceedance diagnostic plots
...
- `ruff` for Python linting and formatting (e.g., `ruff check` and `ruff format`)
- `typstyle` for formatting Typst files
- `pytest` for tests
...
Use the following Makefile as the baseline:

```make
# Definitions
STAGING := /tmp/repo_staging
DOC_CODE := rian_bachelier_presentation
PYTHON ?= python3
VENV := .venv
BIN := $(VENV)/bin
PIP := $(BIN)/pip
...
.PHONY: all build build-typst bundle clean fetch-data format format-python format-typst \
	install lint preprocess analyze figures tables test watch watch-data watch-python watch-typst
...
# Continuously re-run on file changes, keeping outputs up to date.
watch:
...
clean:
	rm -rf $(VENV) .pytest_cache .ruff_cache $(DOC_CODE).pdf bundle*.tar.gz
	mkdir -p output/logs output/figures output/tables
```
