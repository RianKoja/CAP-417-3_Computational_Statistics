...
with open(SECTIONS / "02_recovery.csv", "w", newline="") as f:
    ...
    w.writerow(
        [
            ...
            f"{a_grid_L2:.4f}",
            f"{b_grid_L2:.4f}",
            f"{abs(a_grid_L2 - A_TRUE):.4f}",
            f"{abs(b_grid_L2 - B_TRUE):.4f}",
            f"{t_grid * 1e3:.2f}",
        ]
    )
    w.writerow(
        [
            ...
            f"{a_grid_L1:.4f}",
            f"{b_grid_L1:.4f}",
            f"{abs(a_grid_L1 - A_TRUE):.4f}",
            f"{abs(b_grid_L1 - B_TRUE):.4f}",
            "—",
        ]
    )
    w.writerow(
        [
            ...
            f"{a_ols:.4f}",
            f"{b_ols:.4f}",
            f"{abs(a_ols - A_TRUE):.4f}",
            f"{abs(b_ols - B_TRUE):.4f}",
            f"{t_ols * 1e3:.4f}",
        ]
    )
...
def plot_surface(E, a_range, b_range, a_gt, b_gt, a_min, b_min, title, fname):
    ...
    ax.contour(
        a_range, b_range, E, levels=15, colors="white", linewidths=0.4, alpha=0.5
    )
    ...
    ax.plot(
        ...
        markersize=10,
        markeredgewidth=2.5,
        label=f"Grid min $\\hat{{a}}={a_min:.2f},\\hat{{b}}={b_min:.2f}$",
        zorder=5,
    )
...
ax.plot(
    ...
    "k--",
    linewidth=2,
    label=f"True line ($a={A_TRUE}, b={B_TRUE}$)",
)
ax.plot(
    xp,
    a_ols * xp + b_ols,
