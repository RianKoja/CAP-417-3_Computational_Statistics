...
def generate_linear_data(n_points=100, n_outliers=0, noise_std=0.1, seed=42):
    ...
    if n_outliers > 0:
        ...
        is_outlier = np.concatenate(
            [np.zeros(n_points, dtype=bool), np.ones(n_outliers, dtype=bool)]
        )
    else:
        x = x_inliers
        y = y_inliers
...
def _calculate_mcs_pvalue_cached(losses_tuple, seed=42):
    ...
    if identical_pairs:
        ...
        print(
            f"  Warning: Models with nearly identical losses detected: {identical_pairs}"
        )
        ...
        for model in model_names:
            if mean_losses[model] <= min_loss * 1.0001:  # Within 0.01% of minimum
                ...
                pvalues[model] = max(
                    0.0, 1.0 - (mean_losses[model] - min_loss) / min_loss
                )
...
def evaluate_model(x, y, model, model_type="linear"):
    ...
    try:
        ...
        rmse = np.sqrt(
            mean_squared_error(y_filtered if model_type == "exponential" else y, y_pred)
        )
        mae = mean_absolute_error(
            y_filtered if model_type == "exponential" else y, y_pred
        )
...
def plot_dataset_triplet(datasets_triplet, results_triplet, dataset_type, output_file):
    ...
    fig.suptitle(
        f"{dataset_type} Data: Comparison of LS, RANSAC, LightGBM, and RANSAC+LightGBM",
        fontsize=14,
        y=0.995,
    )
    ...
    for idx, (outlier_level, (x, y, is_outlier)) in enumerate(
        zip(outlier_levels, datasets_triplet)
    ):
        row = idx
        ...
        ax1 = axes[row, 0]
        ax1.scatter(
            x_inliers, y_inliers, alpha=0.6, s=30, c="blue", label="Inliers", marker="o"
        )
        if len(x_outliers) > 0:
            ax1.scatter(
                x_outliers,
        ...
        if idx in results_triplet:
            ls_model, ransac_model, lgbm_model, lgbm_ransac_model, model_type = (
                results_triplet[idx]
            )
            ...
            try:
                ...
                if ransac_model is not None:
                    ...
                    if hasattr(ransac_model_obj, "inlier_mask_"):
                        ...
                        if model_type == "exponential" and hasattr(
                            ransac_model_obj, "full_inlier_mask_"
                        ):
                        ...
                        if np.any(true_positive):
                            ax2.scatter(
                                ...
                                s=40,
                                marker="o",
                                label="Inliers (correctly classified)",
                                edgecolors="darkblue",
                                linewidths=1.5,
                            )
                        if np.any(true_negative):
                            ax2.scatter(
                                ...
                                s=40,
                                marker="x",
                                label="Outliers (correctly classified)",
                                linewidths=2.5,
                            )
...
def create_results_table(datasets_all, results_all, output_file="results_table.tex"):
    ...
    for idx, (dataset_name, (x, y, is_outlier)) in enumerate(
        zip(dataset_names, datasets_all)
    ):
        ...
        if idx in results_all:
            ...
            if (
                ls_metrics["losses"] is not None
                and ransac_metrics["losses"] is not None
                and lgbm_metrics["losses"] is not None
                and lgbm_ransac_metrics["losses"] is not None
            ):
                # Ensure equal length by padding/truncating if needed
                min_len = min(
                    len(ls_metrics["losses"]),
                    len(ransac_metrics["losses"]),
                    len(lgbm_metrics["losses"]),
                    len(lgbm_ransac_metrics["losses"]),
                )
                losses_dict = {
    ...
    with open(output_file, "w") as f:
        ...
        f.write("\\centering\n")
        f.write(
            "\\caption{Comparison of Fitting Methods: RMSE, MAE, and MCS p-values. "
        )
        f.write(
            "Color coding: green indicates better performance, red indicates worse performance. "
        )
        f.write(
            "See Figures~\\ref{fig:demo_linear}, \\ref{fig:demo_quadratic}, and \\ref{fig:demo_exponential} for visualizations.}\n"
        )
        ...
        for idx, data in enumerate(metrics_data):
            if data is not None:
                ...
                rmse_values = [
                    ...
                    data["ransac_rmse"],
                    data["lgbm_rmse"],
                    data["lgbm_ransac_rmse"],
                ]
                mae_values = [
                    data["ls_mae"],
                    data["ransac_mae"],
                    data["lgbm_mae"],
                    data["lgbm_ransac_mae"],
                ]
                mcs_values = [
                    data["ls_mcs"],
                    data["ransac_mcs"],
                    data["lgbm_mcs"],
                    data["lgbm_ransac_mcs"],
                ]
...
def create_confusion_matrix_table(
    datasets_all, results_all, output_file="confusion_matrices.tex"
):
    ...
    for idx, (dataset_name, (x, y, is_outlier)) in enumerate(
        ...
        ls_model, ransac_model, lgbm_model, lgbm_ransac_model, model_type = results_all[
            idx
        ]
        ...
        lgbm_ransac_rates = calc_rates(
            lgbm_ransac_metrics, n_true_outliers, n_true_inliers
        )
    ...
    with open(output_file, "w") as f:
        ...
        f.write(
            "Values in \\%. Color coding: green indicates better performance, red indicates worse.}\n"
        )
        ...
        f.write(
            "\\multicolumn{4}{c|}{RANSAC} & \\multicolumn{4}{c|}{RANSAC+LightGBM} \\\\\n"
        )
        f.write(
        ...
        for data in confusion_data:
            if data is not None:
                ...
                # RANSAC metrics
                f.write(
                    f"{fmt(data['ransac']['tpr'], inverse=False)} & "
                )  # TPR: higher is better
                f.write(
                    f"{fmt(data['ransac']['fpr'], inverse=True)} & "
                )  # FPR: lower is better
                f.write(
                    f"{fmt(data['ransac']['tnr'], inverse=False)} & "
                )  # TNR: higher is better
                f.write(
                    f"{fmt(data['ransac']['fnr'], inverse=True)} & "
                )  # FNR: lower is better
                # RANSAC+LightGBM metrics
                f.write(
                    f"{fmt(data['lgbm_ransac']['tpr'], inverse=False)} & "
                )  # TPR: higher is better
                f.write(
                    f"{fmt(data['lgbm_ransac']['fpr'], inverse=True)} & "
                )  # FPR: lower is better
                f.write(
                    f"{fmt(data['lgbm_ransac']['tnr'], inverse=False)} & "
                )  # TNR: higher is better
                f.write(
                    f"{fmt(data['lgbm_ransac']['fnr'], inverse=True)} \\\\\n"
                )  # FNR: lower is better
            else:
                f.write("- & - & - & - & - & - & - & - & - & - & - \\\\\n")
...
def main(seed=42):
    ...
    datasets.append(
        generate_exponential_data(n_points=100, n_outliers=0, seed=seed + 6)
    )
    datasets.append(
        generate_exponential_data(n_points=80, n_outliers=20, seed=seed + 7)
    )
    datasets.append(
        generate_exponential_data(n_points=50, n_outliers=50, seed=seed + 8)
    )
    ...
    create_confusion_matrix_table(
        datasets, results, output_file=PATH_HERE / "confusion_matrices.tex"
    )

    print(
        "Done! Check demo_results_*.pdf, results_table.tex, and confusion_matrices.tex"
    )
